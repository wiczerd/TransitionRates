function fit = ModelFitFunc_FixedOmega_AllE_FreeInit_Smooth(unks, select, groupNum, Theta, ThetaTot, Hstates, year, Data_full, omeg1to5, lam_pi)
% ModelFitFunc_FixedOmega_AllE_FreeInit_Smooth — Phase 3 objective with
% free initial distribution and proximity-to-ergodic penalty.
%
% Same as ModelFitFunc_FixedOmega_AllE_FreeInit but adds:
...
%   lam_pi      = penalty weight for init dist proximity to ergodic
%
% If lam_pi = 0, this is identical to ModelFitFunc_FixedOmega_AllE_FreeInit.
...
for theta = 1:Theta
    ...
    % Ergodic distribution (for regularization and init dist penalty)
    if theta == Theta
        J = [1 2 4];
        tm3 = tm(J, J);
        pi3 = Ergodic(tm3);
    ...
    % Proximity-to-ergodic penalty
    pi_penalty = pi_penalty + sum((piVec - ergVec).^2);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Init dist proximity penalty
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
reg_accum = reg_accum + lam_pi * Nobs * pi_penalty;
